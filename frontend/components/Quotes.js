...
import { 
  deleteQuote, toggleVisibility, editQuoteAuthenticity, setHighlightedQuote,
} from '../state/quotesSlice'

export default function Quotes() {
  const quotes = useSelector(store => store.quotesState.quotes)
  const displayAllQuotes = useSelector(store => store.quotesState.displayAllQuotes)
  const highlightedQuote = useSelector(store => store.quotesState.highlightedQuote)
